...
export function MegaMenu({ isOpen, onClose }: MegaMenuProps) {
  ...
  return (
    <motion.div
      ...
      animate="visible"
      exit="exit"
      className="fixed inset-0 z-40 bg-[#0A192F] text-[#F0F4F8] flex flex-col pt-24 pb-8 px-6 md:px-12 lg:px-24 overflow-y-auto"
    >
      <div className="flex justify-between items-center pb-8 border-b border-white/10">
        <span className="font-sans text-[10px] tracking-[0.4em] text-[#D4AF37] uppercase">
          Turk Interiors / Premium Studio
        </span>
        <button
          onClick={onClose}
          className="p-2 text-[#E2E8F0] hover:text-[#D4AF37] transition-colors rounded-full border border-white/10 hover:border-[#D4AF37]/40"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 py-12 flex-grow">
        ...
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          {serviceCategories.map((category, idx) => (
            <motion.div variants={itemVariants} key={idx} className="flex flex-col">
              <h3 className="font-serif text-[#D4AF37] text-lg tracking-wider mb-6 border-b border-white/5 pb-2">
                {category.title}
              </h3>
                ...
                {category.links.map((link, lIdx) => (
                  <li key={lIdx}>
                    <Link
                      href={link.href}
                      onClick={onClose}
                      className="font-sans text-[#E2E8F0] hover:text-[#D4AF37] transition-all text-sm tracking-wide flex items-center group"
                    >
                      <span className="relative">
                        {link.name}
                        <span className="absolute bottom-0 left-0 w-0 h-[1px] bg-[#D4AF37] transition-all group-hover:w-full" />
                      </span>
                      <ArrowRight className="w-3.5 h-3.5 ml-2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-[#D4AF37]" />
                    </Link>
                  </li>
                ))}
              </ul>
        ...
        <motion.div
          ...
          className="lg:col-span-4 flex flex-col justify-between bg-[#112240] p-8 rounded-lg border border-white/5"
        >
          <div className="mb-8">
            <h4 className="font-sans text-[10px] text-[#D4AF37] tracking-widest uppercase mb-4">
              Featured Project
            </h4>
            ...
            <h5 className="font-serif text-[#F0F4F8] text-md mb-2">
          ...
          <div className="flex flex-col gap-3 pt-6 border-t border-white/5 text-[#E2E8F0] text-xs">
      ...
      <div className="pt-6 border-t border-white/5 flex flex-col md:flex-row justify-between text-[#E2E8F0]/40 text-[10px] tracking-widest font-sans uppercase gap-4">
        <span>© 2026 Turk Interiors. All rights reserved.</span>
        <div className="flex gap-6">
          <Link href="/privacy" onClick={onClose} className="hover:text-[#D4AF37]">
            Privacy Policy
          </Link>
          <Link href="/terms" onClick={onClose} className="hover:text-[#D4AF37]">
            Terms of Service
          </Link>
        </div>
      </div>
