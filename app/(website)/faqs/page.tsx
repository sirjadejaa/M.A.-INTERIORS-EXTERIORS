  ...
  return (
    <div className="bg-[#F8F7F4] pt-32 pb-24 md:pt-40 md:pb-32">
      <div className="max-w-4xl mx-auto px-6">
        ...
        {/* FAQs Accordion */}
        <div className="flex flex-col border-t border-[#ECE8E2]">
          {mockFAQs.map((faq, idx) => {
            const isOpen = activeIndex === idx;
            return (
              <div key={idx} className="border-b border-[#ECE8E2]">
                <button
                  onClick={() => toggleFAQ(idx)}
                  className="w-full py-6 flex justify-between items-center text-left text-[#111111] hover:text-[#A67C52] transition-colors focus:outline-none"
                >
                  <div className="flex flex-col gap-1.5 pr-6">
                    <span className="font-sans text-[8px] text-[#A67C52] tracking-wider uppercase font-semibold">
                      {faq.category}
                    </span>
                  ...
                  <div className="shrink-0 text-[#A67C52] p-1 border border-[#ECE8E2] rounded-full">
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <div className="pb-8 font-sans text-xs text-[#555555] leading-relaxed max-w-2xl pr-4">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
